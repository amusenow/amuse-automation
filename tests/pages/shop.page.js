...
class ShopPage extends Page {
    ...
    async clickOnProduct() {
        ...
        console.log((await cards).length)
        var name = (await cards)[4].$('.sf-product-card__link')
        var image = (await cards)[4].$('.sf-product-card__image-wrapper')
        var classification = (await cards)[1].$('.flex.flex-row.items-center.text-xxs')
        var brand = (await cards)[4].$('.a-brand.text-xxs')
        var price = (await cards)[4].$('.flex.items-center.justify-between')
